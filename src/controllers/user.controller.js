...
const getUser = catchAsync(async (req, res) => {
  ...
  //we will use this "data" to return address's OR user's data
  let data;
 
  //part to get address only, for a user
  if(req.query.q=="address"){
    // we can not return only address using getUserAddressById, need to return email also.
     data= await userService.getUserAddressById(req.params.userId);
  }
  else{
      //part to get the whole user
    data = await userService.getUserById(req.params.userId);
  }

//  if no user/address found
  if(!data){
    throw new ApiError(httpStatus.NOT_FOUND,"User not found")
  }
   //check if some user is using the jwt token of some other user:
   if(data.email !== req.user.email){
    throw new ApiError(httpStatus.FORBIDDEN, "User not Authenticated to see other user's data");
  }
  // finally retunr user/address if no issue
  // else{
    if(req.query.q=="address"){
